...
- 私有 ACR 命名空间，以及 `music-to-heart`、`qujing-postgres` 两个仓库。
- GitHub 仓库允许 Actions 写入 Packages；ACR 凭证允许 Actions 推送、ECS 拉取。
...
sudo env \
  ...
  ACR_REGISTRY='registry.cn-REGION.aliyuncs.com' \
  ACR_USERNAME='ACR 用户名' \
  ACR_PASSWORD='ACR 拉取密码' \
  ACR_PREFLIGHT_IMAGE='registry.cn-REGION.aliyuncs.com/命名空间/music-to-heart@sha256:已存在的摘要' \
  POSTGRES_IMAGE='registry.cn-REGION.aliyuncs.com/命名空间/qujing-postgres@sha256:已验证的摘要' \
...
不要把 ACR、OSS 密码粘贴到聊天、仓库或 Actions 日志。推荐在 ECS 控制台通过隐藏输入读取密码，再调用 bootstrap，避免密码进入 shell history。

Bootstrap 会连续三次从 ACR 拉取 `ACR_PREFLIGHT_IMAGE`。任何一次失败都会终止初始化；不得强行发布。`POSTGRES_IMAGE` 也必须包含 `@sha256:`，数据库不会接触 Docker Hub。
...
先手动运行 **Mirror PostgreSQL to ACR**，确认工作流输出的源/目标 digest 一致。正常发布只需合并通过检查的 PR：同一次 Buildx 构建推送 GHCR 与 ACR，digest 不一致时发布立即失败。部署记录保存在 GitHub Deployment 与 ECS `/opt/qujing/releases.log`。
